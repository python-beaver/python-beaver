...
class BeaverConfig():
    ...
    def __init__(self, args, logger=None):
        ...
        self._main_defaults = {
            ...
            'mqtt_clientid':  os.environ.get('mosquitto'),
            'mqtt_host':  os.environ.get('MQTT_HOST','localhost'),
            'mqtt_port':  os.environ.get('MQTT_PORT','1883'),
            'mqtt_topic':  os.environ.get('MQTT_TOPIC','/logstash'),
            'mqtt_keepalive':  os.environ.get('MQTT_KEEPALIVE','60'),
            'rabbitmq_host': os.environ.get('RABBITMQ_HOST', 'localhost'),
            'rabbitmq_port': os.environ.get('RABBITMQ_PORT', '5672'),
            'rabbitmq_ssl':  os.environ.get('RABBITMQ_SSL','0'),
            'rabbitmq_ssl_key':  os.environ.get('RABBITMQ_SSL_KEY',''),
            'rabbitmq_ssl_cert':  os.environ.get('RABBITMQ_SSL_CERT',''),
            'rabbitmq_ssl_cacert':  os.environ.get('RABBITMQ_SSL_CACERT',''),
            ...
            'rabbitmq_timeout':  os.environ.get('RABBITMQ_TIMEOUT','1'),
            ...
            'sns_aws_access_key':  os.environ.get('SNS_AWS_ACCESS_KEY',''),
            'sns_aws_secret_key':  os.environ.get('SNS_AWS_SECRET_KEY',''),
            'sns_aws_profile_name':  os.environ.get('SNS_AWS_PROFILE_NAME',''),
            'sns_aws_region':  os.environ.get('SNS_AWS_REGION','us-east-1'),
            'sns_aws_topic_arn':  os.environ.get('SNS_AWS_TOPIC_ARN',''),
            'sqs_aws_access_key':  os.environ.get('SNS_AWS_ACCESS_KEY',''),
            'sqs_aws_secret_key':  os.environ.get('SNS_AWS_SECRET_KEY',''),
            'sqs_aws_profile_name':  os.environ.get('SNS_AWS_PROFILE_NAME',''),
            'sqs_aws_region':  os.environ.get('SQS_AWS_REGION','us-east-1'),
            'sqs_aws_queue':  os.environ.get('SQS_AWS_QUEUE',''),
            'sqs_aws_queue_owner_acct_id':  os.environ.get('SQS_AWS_QUEUE_OWNER_ACCT_ID',''),
            'sqs_bulk_lines': os.environ.get('SQS_BULK_LINES',False),
            'kinesis_aws_access_key':  os.environ.get('KINESIS_AWS_ACCESS_KEY',''), 
            'kinesis_aws_secret_key':  os.environ.get('KINESIS_AWS_SECRET_KEY',''), 
            'kinesis_aws_region':  os.environ.get('KINESIS_AWS_REGION','us-east-1'), 
            'kinesis_aws_stream':  os.environ.get('KINESIS_AWS_STREAM',''), 
            'kinesis_aws_batch_size_max':  os.environ.get('KINESIS_AWS_BATCH_SIZE_MAX','512000'),
            'tcp_host':  os.environ.get('TCP_HOST','127.0.0.1'),
            'tcp_port':  os.environ.get('TCP_PORT','9999'),
            'tcp_ssl_enabled':  os.environ.get('TCP_SSL_ENABLED','0'),
            'tcp_ssl_verify':  os.environ.get('TCP_SSL_VERIFY','0'),
            'tcp_ssl_cacert':  os.environ.get('TCP_SSL_CACERT',''),
            'tcp_ssl_cert':  os.environ.get('TCP_SSL_CERT',''),
            'tcp_ssl_key': os.environ.get('TCP_SSL_KEY',''),
            ...
            'zeromq_pattern':  os.environ.get('ZEROMQ_PATTERN','push'),
            ...
            'stomp_password' : os.environ.get('STOMP_PASSWORD',None),
            'stomp_queue' :  os.environ.get('STOMP_QUEUE','queue/logstash'),

            # exponential backoff
            'respawn_delay':  os.environ.get('RESPAWN_DELAY','3'),
            'max_failure':  os.environ.get('MAX_FAILURE','7'),

            # consumer processes
            'number_of_consumer_processes':  os.environ.get('NUMBER_OF_CONSUMER_PROCESSES','1'),

            # interprocess queue max size before puts block
            'max_queue_size':  os.environ.get('MAX_QUEUE_SIZE','100'),

            # time in seconds before updating the file mapping
            'update_file_mapping_time': '',  # deprecated
            'discover_interval':  os.environ.get('DISCOVER_INTERVAL','15'),

            # time in seconds from last command sent before a queue kills itself
            'queue_timeout':  os.environ.get('QUEUE_TIMEOUT','60'),

            # kill and respawn worker process after given number of seconds
            'refresh_worker_process':  os.environ.get('REFRESH_WORKER_PROCESS',''),

            # time in seconds to wait on queue.get() block before raising Queue.Empty exception
            'wait_timeout':  os.environ.get('WAIT_TIMEOUT','5'),

            # path to sincedb sqlite db
            'sincedb_path':  os.environ.get('SINCEDB_PATH',''),

            # 0 for logstash version < 1.2, 1 for logstash >= 1.2
            'logstash_version':  os.environ.get('LOGSTASH_VERSION',''),

            # ssh tunnel support
            'ssh_key_file':  os.environ.get('SSH_KEY_FILE',''),
            'ssh_tunnel':  os.environ.get('SSH_TUNNEL',''),
            'ssh_tunnel_port':  os.environ.get('SSH_TUNNEL_PORT',''),
            'ssh_remote_host':  os.environ.get('SSH_REMOTE_HOST',''),
            'ssh_remote_port':  os.environ.get('SSH_REMOTE_PORT',''),
            'ssh_options':  os.environ.get('SSH_OPTIONS',''),
            'subprocess_poll_sleep':  os.environ.get('SUBPROCESS_POLL_SLEEP','1'),
